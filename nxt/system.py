...
def _parse_find(tgram):
    tgram.check_status()
    handle = tgram.parse_u8()
    fname = tgram.parse_string(20).strip(b'\x00')
    n_bytes = tgram.parse_u32()
    return (handle, fname.decode(), n_bytes)
...
def _parse_get_device_info(tgram):
    tgram.check_status()
    name = tgram.parse_string(15).rstrip(b'\x00')
    a0 = tgram.parse_u8()
    a1 = tgram.parse_u8()
    a2 = tgram.parse_u8()
    a3 = tgram.parse_u8()
    ...
    return (name.decode(), address, signal_strength, user_flash)

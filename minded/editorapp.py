...
class MindEdDocument:
    def __init__(self, documenturl):
        ...
        '''file.ext'''
        self.shortname = pathlib.Path(parsed.path).name
        #print('shortname: %s' % self.shortname)
        '''/path/to/the'''
        self.filepath = str(pathlib.Path(parsed.path).parent)
    ...
    def update_documenturl(self):
        self.documenturl = pathlib.Path(self.filepath, self.shortname).as_uri() 

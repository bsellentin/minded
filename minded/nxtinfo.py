...
class NXTInfo(object):
    ...
    def get_brickinfo(self):
        ...
        if self.brick_type == 'ev3':
            ...
            fw_version = self.app.ev3brick.get_fw_version()
            self.widgetlist[4].set_text(fw_version)
            self.widgetlist[4].set_xalign(0)
            
            volts = self.app.ev3brick.get_vbatt()
            #logger.debug('Battery level %f V' % volts)
            #self.widgetlist[2].set_text(('%f V' % volts))
            #self.widgetlist[2].set_xalign(0)
